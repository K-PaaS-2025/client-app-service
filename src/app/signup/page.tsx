...
export default function SignupPage() {
  ...
  const [state, formAction, pending] = useActionState(signup, { status: 0, message: "" });
  ...
  const isFormValid = email && password && confirmPassword && passwordsMatch && agreeTerms;
    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-100 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-amber-900 mb-2">회원가입</h1>
          <p className="text-lg text-amber-700">새로운 계정을 만들어보세요</p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl p-8 border border-amber-200">
          <form action={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <p className="text-sm text-amber-600 mt-2">8자 이상, 영문과 숫자를 포함해주세요</p>
            </div>

            {/* 비밀번호 확인 */}
            <div>
              <label htmlFor="confirmPassword" className="block text-lg font-semibold text-amber-900 mb-3">
                비밀번호 확인
              </label>
              <div className="relative">
                <input
                  ...
                  className={`w-full px-6 py-4 text-lg border-2 rounded-2xl focus:ring-4 focus:ring-amber-200 focus:outline-none transition-colors bg-amber-50 pr-14 ${
              ...
              {confirmPassword && !passwordsMatch && (
                <p className="text-sm text-red-600 mt-2">비밀번호가 일치하지 않습니다</p>
              )}
              {confirmPassword && passwordsMatch && (
            ...
            <div className="flex items-start space-x-3">
              <input
                ...
                className="w-5 h-5 mt-1 text-amber-600 border-2 border-amber-300 rounded focus:ring-amber-500 focus:ring-2"
                required
              />
              <label htmlFor="agreeTerms" className="text-base text-amber-900 leading-relaxed">
                <Link href="/terms" className="text-amber-700 hover:text-amber-900 underline font-medium">
                  이용약관
                </Link>
                {" "}및{" "}
                <Link href="/privacy" className="text-amber-700 hover:text-amber-900 underline font-medium">
                  개인정보처리방침
                </Link>
                에 동의합니다
              </label>
            ...
            {/* 에러 메시지 */}
            {(clientError || (state.status !== 0 && state.message)) && (
            ...
            <div className="text-center">
              <p className="text-lg text-amber-700 mb-4">이미 계정이 있으신가요?</p>
              <Link
                href="/login"
